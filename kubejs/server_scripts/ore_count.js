...
onEvent('item.right_click', event => {
	...
	//event.server.runCommand(`summon shulker ${Math.floor(orebserver.currentX)} ${Math.floor(orebserver.currentY)} ${Math.floor(orebserver.currentZ)} {PersistenceRequired:1b,NoAI:1b,NoGravity:1b,Silent:1b,Invulnerable:1b,Glowing:1b,AttachFace:0b,Tags:["orebserver"]}`)
	let blockCounts = {}
	...
	event.server.scheduleInTicks(1, event => {
		...
		for (let i = 0; i < searchSettings.speed;i++) {
			...
			if (!(newBlock === 'end')) {
				...
				//console.log(`X: ${newBlock.x}, Y: ${newBlock.y}, Z: ${newBlock.z}`)					
			} else {
				//event.server.runCommand('kill @e[type=minecraft:shulker,tag=orebserver]')
				let totalBlocks = 0
				event.server.tell('\nTotal block counts:')
		...
		if (rescheduling) {
			//sevent.server.runCommand(`tp @e[type=minecraft:shulker,tag=orebserver] ${orebserver.currentBlock.x} ${orebserver.currentBlock.y} ${orebserver.currentBlock.z}`)
			event.reschedule()
		}
	})
})